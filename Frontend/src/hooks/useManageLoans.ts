...
const useManageLoans = () => {
    ...
    const markBookAsReturned = async (inventoryNumber: number, alertButtons: AlertButton[]) => {
        const result = await LoanService.setLoanStatusReturned(inventoryNumber, contextState.accessToken as string);
