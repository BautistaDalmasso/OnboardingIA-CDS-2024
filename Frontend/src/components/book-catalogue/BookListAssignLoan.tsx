...
interface BookListItemProps {
  ...
  isBookLoaned: (isbn: string) => boolean;
  handleLoanConfirmed: (book: IBookWithLicence) => void;
  handleButtonSearch: (filterCategory: string, searchValue: string) => void;
}

const BookListAssignLoan = ({
  book,
  isBookLoaned,
  handleLoanConfirmed,
  handleButtonSearch,
    ...
    <>
      ...
      <TouchableOpacity
        style={[
          styles.button,
          {
            backgroundColor: isBookLoaned(book.book_data.isbn)
              ? "#ccc"
              : "#007bff",
          },
        ]}
        onPress={() => handleLoanConfirmed(book)}
        disabled={isBookLoaned(book.book_data.isbn)}
      >
        <Text style={styles.buttonText}>
          {isBookLoaned(book.book_data.isbn)
            ? "Prestamo realizado"
            : "Asignar prestamo"}
        </Text>
      </TouchableOpacity>
...
export default BookListAssignLoan;
